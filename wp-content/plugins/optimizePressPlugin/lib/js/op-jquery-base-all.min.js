! function() {
    if (jQuery && !window.opjq) {
        ...
        c = c.replace(/(\d+\.\d+).+/i, "$1"), c = c.split(/\./gi), a = c[0], b = c[1], (1 > a || 8 > b && console && console.error) && console.error("Currently is loaded jQuery version " + jQuery.fn.jquery + " which is too low for OptimizePress. Please check if you're using WordPress version 3.5 or higher or contact customer support."), window.opjq = jQuery.noConflict()
    }
...
function(a) {
    a.loadScript = function(b, c, d) {
        var e = !1,
            f = null;
        a.isFunction(c) ? (f = c, e = d || e) : (e = c || e, f = d || f);
        var g = !0;
        a('script[type="text/javascript"]').each(function() {
            return g = b != a(this).attr("src")
        }), g ? a.ajax({
            type: "GET",
            url: b,
            success: f,
...
function(a, b, c, d) {
    ...
    c.extend(h, {
        ...
        showLoading: function() {
            var a, b;
            h.hideLoading(), a = c('<div id="fancybox-loading"></div>').click(h.cancel).appendTo("body"), g.bind("keydown.loading", function(a) {
...
}(window, document, opjq), opjq(document).ready(function(a) {
        var b = a("body");
        window.op_show_loading = function() {
            window.op_dont_show_loading || a("#op_overlay").length > 0 || (b.append('<div id="op_loading"></div><div id="op_overlay"></div>'), setTimeout(function() {
                a("#op_overlay, #op_loading").css("opacity", 1)
            }, 100))
        }, window.op_hide_loading = function(b) {
            window.op_dont_hide_loading || (b && a("#op_overlay, #op_loading").remove(), window.top.opjq.fancybox && window.top.opjq.fancybox.hideLoading(), a("#op_overlay, #op_loading").css("opacity", 0), setTimeout(function() {
                a("#op_overlay, #op_loading").remove()
            }, 200))
        }
    }),
    function(a, b, c, d) {
        function e(b, c) {
            this.element = b, this.options = a.extend({}, j, c), this._defaults = j, this._name = h, this.init()
        }
        ...
        g() || (k = !1), e.prototype = {
            init: function() {
                var b = a(this.element),
                    e = this,
                    g = !0;
                if (0 == e.options.touchDevices && f() && (g = !1), c.all && !c.querySelector && (g = !1), 1 == g) {
                    if (1 == this.options.iconDesktop && !f() || 1 == this.options.iconTouch && f()) {
                        var h = b.attr("title");
                        b.removeAttr("title");
                        var i = e.options.iconTheme,
                            j = a('<span class="' + i.replace(".", "") + '" title="' + h + '">' + this.options.icon + "</span>");
                        j.insertAfter(b), b.data("tooltipsterIcon", j), b = j
                    }
                    var k = a.trim(e.options.content).length > 0 ? e.options.content : b.attr("title");
                    b.data("tooltipsterContent", k), b.removeAttr("title"), 1 == this.options.touchDevices && f() ? b.bind("touchstart", function(a, b) {
                        e.showTooltip()
            ...
            showTooltip: function(b) {
                var c = a(this.element),
                    e = this;
                c.data("tooltipsterIcon") !== d && (c = c.data("tooltipsterIcon")), c.hasClass("tooltipster-disable") || (a(".tooltipster-base").not(".tooltipster-dying").length > 0 && 1 == e.options.onlyOne && a(".tooltipster-base").not(".tooltipster-dying").not(c.data("tooltipster")).each(function() {
                    ...
                    e.options.functionBefore(c, function() {
                        if (c.data("tooltipster") !== d && "" !== c.data("tooltipster")) {
                            ...
                            if (!b.hasClass("tooltipster-kill")) {
                                ...
                                if (b.removeClass("tooltipster-dying"), 1 == k && b.clearQueue().addClass(h + "-show"), e.options.timer > 0) {
                                    var i = b.data("tooltipsterTimer");
                                    clearTimeout(i), i = setTimeout(function() {
                                        b.data("tooltipsterTimer", d), e.hideTooltip()
                                    }, e.options.timer), b.data("tooltipsterTimer", i)
                                }
                                1 == e.options.touchDevices && f() && a("body").bind("touchstart", function(b) {
                            ...
                            var j = c.data("tooltipsterContent"),
                                l = e.options.theme,
                                m = l.replace(".", ""),
                                h = "tooltipster-" + e.options.animation,
                                n = "-webkit-transition-duration: " + e.options.speed + "ms; -webkit-animation-duration: " + e.options.speed + "ms; -moz-transition-duration: " + e.options.speed + "ms; -moz-animation-duration: " + e.options.speed + "ms; -o-transition-duration: " + e.options.speed + "ms; -o-animation-duration: " + e.options.speed + "ms; -ms-transition-duration: " + e.options.speed + "ms; -ms-animation-duration: " + e.options.speed + "ms; transition-duration: " + e.options.speed + "ms; animation-duration: " + e.options.speed + "ms;",
                                o = e.options.fixedWidth > 0 ? "width:" + e.options.fixedWidth + "px;" : "",
                                p = e.options.maxWidth > 0 ? "max-width:" + e.options.maxWidth + "px;" : "",
                                q = 1 == e.options.interactive ? "pointer-events: auto;" : "",
                                b = a('<div class="tooltipster-base ' + m + " " + h + '" style="' + o + " " + p + " " + q + " " + n + '"><div class="tooltipster-content">' + j + "</div></div>");
                            b.appendTo("body"), c.data("tooltipster", b), b.data("origin", c), e.positionTooltip(), e.options.functionReady(c, b), 1 == k ? b.addClass(h + "-show") : b.css("display", "none").removeClass(h).fadeIn(e.options.speed);
                            var r = j,
                                s = setInterval(function() {
                                    var d = c.data("tooltipsterContent");
                                    0 == a("body").find(c).length ? (b.addClass("tooltipster-dying"), e.hideTooltip()) : r !== d && "" !== d && (r = d, b.find(".tooltipster-content").html(d), 1 == e.options.updateAnimation && (g() ? (b.css({
                                        width: "",
                                        ...
                                        b.removeClass("tooltipster-content-changing"), setTimeout(function() {
                                            b.css({
                                    ...
                                    })), e.positionTooltip()), (0 == a("body").find(b).length || 0 == a("body").find(c).length) && clearInterval(s)
                                }, 200);
                            if (e.options.timer > 0) {
                                var i = setTimeout(function() {
                                    b.data("tooltipsterTimer", d), e.hideTooltip()
                                }, e.options.timer + e.options.speed);
                                b.data("tooltipsterTimer", i)
                            }
                            1 == e.options.touchDevices && f() && a("body").bind("touchstart", function(b) {
                                if (1 == e.options.interactive) {
                                    var c = a(b.target),
            ...
            hideTooltip: function(b) {
                ...
                if (f == d && (f = a(".tooltipster-dying")), c.clearQueue(), f !== d && "" !== f) {
                    ...
                    1 == k ? f.clearQueue().removeClass(h + "-show").addClass("tooltipster-dying").delay(e.options.speed).queue(function() {
                        f.remove(), c.data("tooltipster", ""), a("body").css("verflow-x", ""), e.options.functionAfter(c)
            ...
            positionTooltip: function(c) {
                ...
                function f(c, d) {
                    r - a(b).scrollTop() - p - N - 12 < 0 && d.indexOf("top") > -1 && (h.options.position = c, t = d), r + m + p + 12 + N > a(b).scrollTop() + a(b).height() && d.indexOf("bottom") > -1 && (h.options.position = c, t = d, M = r - p - N - 12)
                }
                var g = a(this.element),
                    h = this;
                if (g.data("tooltipsterIcon") !== d && (g = g.data("tooltipsterIcon")), g.data("tooltipster") !== d && "" !== g.data("tooltipster")) {
                    ...
                    if (g.is("area")) {
                        var u = g.attr("shape"),
                            v = g.parent().attr("name"),
                            w = a('img[usemap="#' + v + '"]'),
                            x = w.offset().left,
                            y = w.offset().top,
                            z = g.attr("coords") !== d ? g.attr("coords").split(",") : d;
                        if ("circle" == u) {
                            var A = parseInt(z[0]),
                                B = parseInt(z[1]),
                                C = parseInt(z[2]);
                            ...
                            for (i = 0; i < z.length; i++) {
                                var K = parseInt(z[i]);
                                "even" == J ? (K > H && (H = K, 0 == i && (F = H)), F > K && (F = K),
                                    J = "odd") : (K > I && (I = K, 1 == i && (G = I)), G > K && (G = K), J = "even")
                            }
                    ...
                    var L = 0,
                        M = 0,
                        N = parseInt(h.options.offsetY),
                        O = parseInt(h.options.offsetX),
                        P = "";
                    if ("top" == h.options.position) {
                        var Q = s + n - (s + l);
                        L = s + O - Q / 2, M = r - p - N - 12, e(), f("bottom", "top")
                    }
                    if ("top-left" == h.options.position && (L = s + O, M = r - p - N - 12, e(), f("bottom-left", "top-left")), "top-right" == h.options.position && (L = s + l + O - n, M = r - p - N - 12, e(), f("bottom-right", "top-right")), "bottom" == h.options.position) {
                        var Q = s + n - (s + l);
                        L = s - Q / 2 + O, M = r + m + N + 12, e(), f("top", "bottom")
                    }
                    if ("bottom-left" == h.options.position && (L = s + O, M = r + m + N + 12, e(), f("top-left", "bottom-left")), "bottom-right" == h.options.position && (L = s + l + O - n, M = r + m + N + 12, e(), f("top-right", "bottom-right")), "left" == h.options.position) {
                        L = s - O - n - 12, myLeftMirror = s + O + l + 12;
                        var R = r + p - (r + g.outerHeight(!1));
                        if (M = r - R / 2 - N, 0 > L && myLeftMirror + n > k) {
                            var S = 2 * parseFloat(j.css("border-width")),
                                T = n + L - S;
                            j.css("width", T + "px"), p = j.outerHeight(!1), L = s - O - T - 12 - S, R = r + p - (r + g.outerHeight(!1)), M = r - R / 2 - N
                        } else 0 > L && (L = s + O + l + 12, j.data("arrow-reposition", "left"))
                    }
                    if ("right" == h.options.position) {
                        ...
                        if (M = r - R / 2 - N, L + n > k && myLeftMirror < 0) {
                            var S = 2 * parseFloat(j.css("border-width")),
                                T = k - L - S;
                            j.css("width", T + "px"), p = j.outerHeight(!1), R = r + p - (r + g.outerHeight(!1)), M = r - R / 2 - N
                        } else L + n > k && (L = s - O - n - 12, j.data("arrow-reposition", "right"))
                    }
                    if (1 == h.options.arrow) {
                        var U = "tooltipster-arrow-" + h.options.position;
                        if (h.options.arrowColor.length < 1) var V = j.css("background-color");
                        else var V = h.options.arrowColor;
                        var W = j.data("arrow-reposition");
                        if (W ? "left" == W ? (U = "tooltipster-arrow-right", W = "") : "right" == W ? (U = "tooltipster-arrow-left", W = "") : W = "left:" + W + "px;" : W = "", "top" == h.options.position || "top-left" == h.options.position || "top-right" == h.options.position) var X = parseFloat(j.css("border-bottom-width")),
                            Y = j.css("border-bottom-color");
                        else if ("bottom" == h.options.position || "bottom-left" == h.options.position || "bottom-right" == h.options.position) var X = parseFloat(j.css("border-top-width")),
                            Y = j.css("border-top-color");
                        else if ("left" == h.options.position) var X = parseFloat(j.css("border-right-width")),
                            Y = j.css("border-right-color");
                        else if ("right" == h.options.position) var X = parseFloat(j.css("border-left-width")),
                            Y = j.css("border-left-color");
                        else var X = parseFloat(j.css("border-bottom-width")),
                            Y = j.css("border-bottom-color");
                        X > 1 && X++;
                        var Z = "";
                        if (0 !== X) {
                            var $ = "",
                                _ = "border-color: " + Y + ";"; - 1 !== U.indexOf("bottom") ? $ = "margin-top: -" + X + "px;" : -1 !== U.indexOf("top") ? $ = "margin-bottom: -" + X + "px;" : -1 !== U.indexOf("left") ? $ = "margin-right: -" + X + "px;" : -1 !== U.indexOf("right") && ($ = "margin-left: -" + X + "px;"), Z = '<span class="tooltipster-arrow-border" style="' + $ + " " + _ + ';"></span>'
                        }
                        j.find(".tooltipster-arrow").remove(), P = '<div class="' + U + ' tooltipster-arrow" style="' + W + '">' + Z + '<span style="border-color:' + V + ';"></span></div>', j.append(P)
                    }
                    j.css({
                        top: M + "px",
                        left: L + "px"
                ...
                return c.each(function() {
                    switch (b.toLowerCase()) {
                        ...
                        case "destroy":
                            ...
                            break;
                        case "update":
                            a(this).data("tooltipsterContent", g);
                            break;
                        case "reposition":
